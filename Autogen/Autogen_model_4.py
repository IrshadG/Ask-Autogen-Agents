...
#conda activate POP
#streamlit run Langchain/Autogen_model_4.py
...
""" + str_code
        ...
        self.user_proxy.execute_code_blocks([("sh", script)])
        # return str_code
    ...
    def trying_example(self):
        ...
        chat = self.user_proxy.chat_messages[self.assistant]
        output = chat[len(chat)-1]['content']
        return output
...
if uploaded_file:
    ...
    if(problem):
        ...
        if st.button('Submit'):
            ...
            #st.write(output)
            st.write(problem)
            st.write(str(uploaded_content.to_dict()))